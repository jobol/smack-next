...
static int audit_receive_msg(struct sk_buff *skb, struct nlmsghdr *nlh)
{
	...
	case AUDIT_SIGNAL_INFO:
		len = 0;
		if (secid_valid(&audit_sig_sid, __func__)) {
		    ...
		    const struct path *path, int record_num, int *call_panic)
{
	...
	if (n->ino != AUDIT_INO_UNSET)
		audit_log_format(ab, " inode=%lu"
				 ...
				 MAJOR(n->rdev),
				 MINOR(n->rdev));
	if (secid_valid(&n->osid, __func__)) {
		char *ctx = NULL;
		u32 len;
 ...
 */
void audit_log_secctx(struct audit_buffer *ab, struct secids *secid)
{
	u32 len;
	char *secctx;
